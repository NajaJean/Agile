...
public class DatabaseDataTest {
	...
	@Before
	public void init() {
		client = new Client("M", "1234", "Mathilde","mathildesemail@gmail.com","Anker Egelundsvej 1");
	...
	public void dataObserverTest() {
		...
		int oldLengthContainers = DatabaseData.getContainers().length;
		Container C = new Container(DatabaseData.getLocations()[1]);
		DatabaseData.getDatabase().addToDatabase("Containers", C.toString(), C);
		
		assertEquals((oldLengthContainers + 1), DatabaseData.getContainers().length);
		assertEquals(C, DatabaseData.getContainers()[oldLengthContainers]);
		
		/*
		int oldLengthContainerJourney = DatabaseData.getJournies().length;
		ContainerJourney newContainerJourney = new ContainerJourney(startLocation, endLocation, newContainer);
		DatabaseData.getDatabase().addToDatabase("Journies", newContainerJourney.toString(), newContainerJourney);
		
		assertEquals((oldLengthContainerJourney + 1), DatabaseData.getJournies().length);
		assertEquals(newContainerJourney, DatabaseData.getJournies()[oldLengthContainerJourney]);
		*/
	}
}
