...
Feature: ClientUserManagment
  ...
  Given a client with username "Corona" and password "1234" registers a container
  And the client sets the contents of the container
  When clients log into the Container Management System
  Then only the client with username "Corona" and password "1234" has access to that container
